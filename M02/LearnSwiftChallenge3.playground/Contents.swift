...
class Library {
    ...
    func searchByTitle(_ title:String) -> String {
        ...
        if catalogue.contains(where: { $0.value.title == title}) {
            if !checkedOutBooks.contains(where: {$0.key == title}) {
                return "Available"
            }else {
                return "Checked out by name " + checkedOutBooks[title]!.name
            }
        }
    ...
    func checkOut(_ bookId:String, _ person:Person) -> String {
        ...
        if catalogue.contains(where: { $0.value.title == bookId}) {
            if checkedOutBooks.contains(where: {$0.key == bookId}) {
